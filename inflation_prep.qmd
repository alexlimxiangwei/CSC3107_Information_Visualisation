...
For the next step, we will merge the data with the data on electricity and gas prices, which are not included in the original data set.

We chose to use Residential Average Price of Electricity in cents per Kilowatt hour for the U.S as we do not want to focus on commercial or industrial pr

```{r}
#| label: merge-electricity-gas
#| 
# Get a list of all the XLSX file paths
